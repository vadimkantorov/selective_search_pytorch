...
class SelectiveSearch(torch.nn.Module):
    ...
    def __init__(self, preset = 'fast', lib_path = os.path.abspath('selectivesearchsegmentation.so'), max_num_rects = 4096, max_num_seg = 16, max_num_bit = 32):
    ...
    def forward(self, img):
        ...
        rects = torch.empty(size = (img.shape[0], self.max_num_rects, 4), dtype = torch.int32)
        seg   = torch.empty(size = (img.shape[0], self.max_num_seg, img.shape[0], img.shape[1]), dtype = torch.int32)
        lab   = torch.empty(size = (img.shape[0], self.max_num_rects, ), dtype = torch.int32)
        bit   = torch.empty(size = (img.shape[0], self.max_num_rects, self.max_num_bit), dtype = torch.int32)
